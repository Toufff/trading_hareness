  ...
  <el-card shadow="never" class="group-relay-status-panel">
    ...
    <template v-else>
      <el-descriptions :column="mobileLayout ? 1 : 5" border size="small">
        ...
        <el-descriptions-item label="远端投递队列"><el-tag :type="groupRelayStatus.delivery_outbox?.failed ? 'danger' : groupRelayStatus.delivery_outbox?.depth ? 'warning' : 'success'">待投递 {{ groupRelayStatus.delivery_outbox?.depth ?? 0 }} · 终止失败 {{ groupRelayStatus.delivery_outbox?.failed ?? 0 }}</el-tag></el-descriptions-item>
