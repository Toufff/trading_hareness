...
const mainWavePositivePattern = computed<Record<string, unknown>[]>(() => {
  const value = nestedValue(latestMainWaveExperiment.value?.metrics, 'pattern_summary.positive_coefficients');
  return Array.isArray(value) ? value as Record<string, unknown>[] : [];
});
        ...
        <template v-if="activeSection === 'research'">
          ...
          <el-tabs v-model="activeResearchTab" class="research-tabs">
            ...
            <el-tab-pane label="因子与回测" name="factor-lab">
              ...
              <el-card shadow="never" class="section-gap"><template #header><div class="card-header"><div><span>观察池主升启动 · Qlib 风格影子模型</span><small class="realtime-refresh-time">一年日线；T 收盘形成先验，T+1 开盘入场标签，未来10日结果；60/20/20时间切分并留10日隔离。</small></div><el-button type="primary" :loading="actionLoading === '训练观察池主升影子模型'" @click="runMainWaveResearch">重新训练</el-button></div></template><el-alert :title="latestMainWaveExperiment ? `当前门禁：${nestedValue(latestMainWaveExperiment.metrics,'promotion_gate.status')}。未晋级时只写实时影子证据，不发飞书、不进入决策分。` : '尚未训练观察池主升模型。'" :type="nestedValue(latestMainWaveExperiment?.metrics,'promotion_gate.status') === 'eligible_for_manual_review' ? 'success' : 'warning'" :closable="false" show-icon/><template v-if="latestMainWaveExperiment"><el-row :gutter="12" class="metric-row section-gap"><el-col :md="4" :xs="12"><el-statistic title="总样本" :value="Number(nestedValue(latestMainWaveExperiment.metrics,'sample_rows') || 0)"/></el-col><el-col :md="4" :xs="12"><el-statistic title="样本外AUC" :value="Number(nestedValue(latestMainWaveExperiment.metrics,'walk_forward.test.roc_auc') || 0)" :precision="3"/></el-col><el-col :md="4" :xs="12"><el-statistic title="Top20%命中" :value="Number(nestedValue(latestMainWaveExperiment.metrics,'walk_forward.test.selected_precision') || 0) * 100" suffix="%" :precision="1"/></el-col><el-col :md="4" :xs="12"><el-statistic title="相对基准Lift" :value="Number(nestedValue(latestMainWaveExperiment.metrics,'walk_forward.test.selected_lift') || 0)" :precision="2"/></el-col><el-col :md="4" :xs="12"><el-statistic title="Top20%后10日" :value="Number(nestedValue(latestMainWaveExperiment.metrics,'walk_forward.test.selected_terminal_return') || 0) * 100" suffix="%" :precision="2"/></el-col><el-col :md="4" :xs="12"><el-statistic title="当前评分股" :value="mainWaveCurrentScores.length"/></el-col></el-row><el-divider content-position="left">训练段学到的条件方向</el-divider><el-space wrap><el-tag v-for="item in mainWavePositivePattern" :key="String(item.feature)" type="success">{{ item.label }} +{{ Number(item.coefficient).toFixed(3) }}</el-tag></el-space><el-divider content-position="left">下一交易日影子先验</el-divider><el-table :data="mainWaveCurrentScores.slice(0, 12)" max-height="330" size="small"><el-table-column prop="rank" label="#" width="46"/><el-table-column prop="symbol" label="代码" width="105"/><el-table-column prop="name" label="名称" min-width="95"/><el-table-column label="模型概率" width="95"><template #default="{ row }">{{ (Number(row.model_score) * 100).toFixed(1) }}%</template></el-table-column><el-table-column label="池内分位" width="90"><template #default="{ row }">{{ (Number(row.percentile) * 100).toFixed(1) }}%</template></el-table-column><el-table-column label="影子状态" min-width="130"><template #default="{ row }"><el-tag size="small" :type="row.state === 'shadow_top_quintile' ? 'warning' : 'info'">{{ row.state === 'shadow_top_quintile' ? '盘中等待确认' : '普通观察' }}</el-tag></template></el-table-column></el-table></template></el-card>
