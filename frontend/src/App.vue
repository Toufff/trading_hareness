...
type PaperStatus = { mode?: string; live_orders?: boolean; decisions?: Record<string, unknown>[]; positions?: Record<string, unknown>[]; latest_portfolio?: Record<string, unknown> | null; risk_events?: Record<string, unknown>[]; boundary?: string };
...
async function loadResearch() {
  ...
  try {
    ...
    const [outcomeData, scorecardData, messageData, skillData, analystResearchData, paperData] = await Promise.all([
    ...
    remoteMessages.value = messageData.items ?? []; analystSkills.value = skillData.items ?? []; analystResearchStatus.value = analystResearchData; paperStatus.value = paperData;
