...
<template>
  <el-container class="app-shell">
    ...
    <el-container>
      <el-header class="topbar"><div><h1>{{ dashboard.activeSection === 'research' ? '量化研究台' : dashboard.activeSection === 'monitor' ? '导入监控' : dashboard.activeSection === 'workbench' ? '飞书工作台' : '手动投递' }}</h1><span>{{ dashboard.activeSection === 'research' ? '分析师证据、市场数据与研究候选池' : dashboard.activeSection === 'workbench' ? '汇总群协作闭环、可用能力与授权状态' : '本地持久化导入链路' }}</span></div><el-button :icon="Refresh" :dashboard.loading="dashboard.activeSection === 'workbench' ? dashboard.feishuWorkbenchLoading : dashboard.loading" @click="dashboard.activeSection === 'workbench' ? dashboard.loadFeishuWorkbench() : dashboard.loadResearch()">刷新数据</el-button></el-header>
  ...
  <el-dialog v-model="dashboard.fetchDialogOpen" title="受控数据读取" width="680px" destroy-on-close><el-form label-position="top"><el-row :gutter="14"><el-col :span="12"><el-form-item label="API"><el-input v-model="dashboard.fetchForm.api_name"/></el-form-item></el-col><el-col :span="12"><el-form-item label="来源"><el-select v-model="dashboard.fetchForm.provider" class="full-width"><el-option label="自动回退" value="auto"/><el-option label="主 Tushare 源" value="primary"/><el-option label="Super 聚合兼容路由" value="super"/><el-option label="Super SDK 完整路径" value="super_sdk"/><el-option label="Super GET 已验证路径" value="super_get"/><el-option label="REST 备用源" value="backup"/></el-select></el-form-item></el-col></el-row><el-alert title="完整 ths_member 请选自动、Super 聚合或 Super SDK；Super GET 对大板块会被上游截断。" type="warning" :closable="false" class="section-gap"/><el-form-item label="参数 JSON"><el-input v-model="dashboard.fetchForm.paramsText" type="textarea" :rows="8" class="mono"/></el-form-item><el-row :gutter="14"><el-col :span="16"><el-form-item label="字段"><el-input v-model="dashboard.fetchForm.fields"/></el-form-item></el-col><el-col :span="8"><el-form-item label="最大行数"><el-input-number v-model="dashboard.fetchForm.max_rows" :min="1" :max="10000" class="full-width"/></el-form-item></el-col></el-row></el-form><template #footer><el-button @click="dashboard.fetchDialogOpen = false">取消</el-button><el-button type="primary" :dashboard.loading="dashboard.actionLoading === 'fetch'" @click="dashboard.executeFetch">读取并保存证据</el-button></template></el-dialog>
  <el-dialog v-model="dashboard.groupRelayRouteDialog" :title="dashboard.groupRelayRouteForm.key ? '编辑源群' : '新增源群'" width="520px" destroy-on-close>
    ...
    <template #footer><el-button @click="dashboard.groupRelayRouteDialog = false">取消</el-button><el-button type="primary" :dashboard.loading="dashboard.groupRelayRouteSaving" @click="dashboard.saveGroupRelayRoute">保存</el-button></template>
  </el-dialog>
  <el-dialog v-model="dashboard.workbenchIntegrationDialog" :title="dashboard.workbenchIntegration.title" width="620px" destroy-on-close>
    ...
    <template #footer><el-button @click="dashboard.workbenchIntegrationDialog = false">取消</el-button><el-button type="primary" :dashboard.loading="dashboard.feishuWorkbenchAction.startsWith('integration:')" @click="dashboard.submitWorkbenchIntegration">提交</el-button></template>
  </el-dialog>
