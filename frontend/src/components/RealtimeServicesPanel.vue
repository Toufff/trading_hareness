...
<template>
  <el-card shadow="never" class="realtime-health-panel">
    ...
    <el-alert v-if="services.edge_handoff?.configured" class="section-gap" :type="services.edge_handoff.state === 'ready' ? 'success' : services.edge_handoff.state === 'catching_up' ? 'warning' : 'warning'" :closable="false" show-icon :title="`远端采集节点 ${services.edge_handoff.state === 'ready' ? '已追平' : services.edge_handoff.state === 'catching_up' ? '正在追平' : '状态待刷新'}；证据最近同步 ${dateText(services.edge_handoff.last_imported_at)}（${ageText(services.edge_handoff.age_seconds)}前），序列 ${services.edge_handoff.sequence ?? 0}/${services.edge_handoff.remote_sequence ?? services.edge_handoff.sequence ?? 0}，积压 ${services.edge_handoff.sequence_lag ?? 0}。本地只做研究/回放，远端采集与本地分析状态分开显示。`"/>
    <el-descriptions v-if="services.edge_handoff?.configured" class="section-gap" title="远端证据同步" :column="3" border size="small">
      <el-descriptions-item label="本地 / 远端序列">{{ services.edge_handoff.sequence ?? 0 }} / {{ services.edge_handoff.remote_sequence ?? services.edge_handoff.sequence ?? 0 }}</el-descriptions-item>
      <el-descriptions-item label="待导入变更">{{ services.edge_handoff.sequence_lag ?? 0 }}{{ services.edge_handoff.has_more ? '（继续分页）' : '' }}</el-descriptions-item>
      <el-descriptions-item label="最近远端变更">{{ dateText(services.edge_handoff.remote_latest_changed_at) }}</el-descriptions-item>
      <el-descriptions-item label="本次同步">{{ services.edge_handoff.pull?.pages_imported ?? 0 }} 页 · {{ services.edge_handoff.pull?.rows_imported ?? 0 }} 行</el-descriptions-item>
      <el-descriptions-item label="同步耗时">{{ services.edge_handoff.pull?.duration_ms ?? '-' }} ms</el-descriptions-item>
      <el-descriptions-item label="同步状态">{{ services.edge_handoff.pull?.state ?? '-' }}</el-descriptions-item>
    </el-descriptions>
    ...
    <el-alert v-if="services.edge_handoff?.runtime?.live_session_acceptance?.state && services.edge_handoff.runtime.live_session_acceptance.state !== 'not_run'" class="section-gap" :type="services.edge_handoff.runtime.live_session_acceptance.state === 'passed' ? 'success' : services.edge_handoff.runtime.live_session_acceptance.state === 'failed' ? 'error' : 'info'" :closable="false" show-icon :title="`远端盘中验收：${services.edge_handoff.runtime.live_session_acceptance.state === 'passed' ? '通过' : services.edge_handoff.runtime.live_session_acceptance.state === 'failed' ? '失败' : '待命'}；检查于 ${dateText(services.edge_handoff.runtime.live_session_acceptance.checked_at)}。${services.edge_handoff.runtime.live_session_acceptance.reason ?? ''}`"/>
