...
const chartOption = computed<EChartsOption>(() => {
  ...
  return { animation: !reducedMotion(), animationDuration: 240, tooltip: { trigger: 'axis', confine: true }, legend: { top: 8, itemWidth: 16, textStyle: { color: '#626d6d', fontSize: 12 }, selected: { '分歧': false } }, grid: { left: 42, right: 16, top: 52, bottom: 40 }, xAxis: { type: 'category', data: labels, axisLabel: { hideOverlap: true, color: '#626d6d', formatter: (value: string) => value.replace(/^\d{4}-/, '').replace(/ 收盘$/, '') }, axisLine: { lineStyle: { color: '#dcd6c5' } }, axisTick: { show: false } }, yAxis: { type: 'value', min: 0, max: 100, splitLine: { lineStyle: { color: '#dcd6c5' } }, axisLabel: { color: '#626d6d' } }, series: [series('关注', 'attention_score', '#2d627c'), series('活跃', 'activity_score', '#836026'), series('方向', 'strength_score', '#387b66'), series('分歧', 'risk_score', '#ae473c')] };
});
