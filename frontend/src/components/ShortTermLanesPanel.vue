    ...
    <template v-if="scan">
      ...
      <template v-else>
        ...
        <section v-if="currentReport" class="report-body" :data-report-view="selected">
          <div class="report-heading"><h2>{{ currentReport.title }}</h2><button class="download" @click="downloadStrategyReport(currentReport)">下载本报告 · Markdown</button></div>
          ...
          <template v-if="selected === 'overview'">
            ...
            <StrategyEffectiveness :value="scan.effectiveness" :lanes="scan.lanes" />
            <ObservationFollowup :followup="scan.followup" />
          </template>
          <template v-else-if="currentLane">
            ...
            <StrategyEffectiveness :value="scan.effectiveness" :lane="currentLane.key" :lanes="scan.lanes" />
            <ObservationFollowup :followup="scan.followup" :lane="currentLane.key" />
          </template>
