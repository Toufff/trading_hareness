...
class IntradayScanSourceStatusTests(unittest.TestCase):
    def test_partial_direct_quotes_do_not_claim_full_decision_coverage(self):
        status = build_scan_source_status(
            selected_symbols=["000001.SZ", "000002.SZ"],
            quotes={
                "000001.SZ": {"price_source": "longhuvip_watch_quote", "price_freshness": {"status": "fresh"}},
                "000002.SZ": {"price_source": "fuyao_ths_all_a_snapshot", "price_freshness": {"status": "fresh"}},
            },
            all_a_rows=[{"symbol": "000001.SZ"}],
            sina_watch_rows=[{"ts_code": "000002.SZ"}], licensed_watch_rows=[{"ts_code": "000001.SZ"}],
            licensed_watch_status={"status": "partial"}, eastmoney_watch_flow_rows=[],
        ...
        direct_watch = status["direct_watch"]
        self.assertEqual(direct_watch["status"], "partial")
        self.assertEqual(direct_watch["decision_eligible_watch_quote_symbols"], 1)
        self.assertEqual(status["fuyao"]["all_a_only_watch_quote_symbols"], 1)
        self.assertEqual(direct_watch["sina_fallback_watch_quote_symbols"], 1)
    ...
    def test_no_direct_or_all_a_rows_is_explicitly_unavailable(self):
        status = build_scan_source_status(
            selected_symbols=["000001.SZ"], quotes={}, all_a_rows=[], sina_watch_rows=[],
        ...
        self.assertEqual(status["direct_watch"]["status"], "unavailable")
        ...
        self.assertEqual(status["direct_watch"]["missing_direct_watch_quote_symbols"], 1)
