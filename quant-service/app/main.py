...
discipline_alert_loop, intraday_advisory_loop = build_notification_loops(database=db, run_database=run_database_blocking,
    ...
    session_open=lambda *args, **kwargs: realtime_market_session_async(*args, **kwargs), dashboard_url=lambda: Settings.from_environ().dashboard_decision_url or Settings.from_environ().dashboard_public_url)
...
app.include_router(build_peer_support_router(
    db,
    shared_read_key=lambda: Settings.from_environ().shared_read_api_key,
    log_directory=lambda: Settings.from_environ().postgres_log_dir,
    allowed_roles=lambda: Settings.from_environ().peer_consumer_roles,
))
