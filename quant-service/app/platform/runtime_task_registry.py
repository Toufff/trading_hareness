...
RUNTIME_TASK_CONTRACTS: Final[dict[str, RuntimeTaskContract]] = {
    "intraday_monitor": RuntimeTaskContract(
        ...
        "intraday_edge",
        "30s during session",
        ("order_book_quote", "a_share_prices_snapshot"),
        ("intraday_scan_runs", "intraday_signal_events"),
        "bounded watchlist scan and research alert writer",
        90,
    ),
    "discipline_alerts": RuntimeTaskContract(
        "discipline_alerts",
        "research",
        "30s during session; settled daily after close",
        ("intraday_minutes", "daily"),
        ("discipline_evaluations", "discipline_alert_events", "discipline_alert_deliveries"),
    ...
    "super_get_fast_quote": RuntimeTaskContract(
        ...
        "intraday_edge",
        "1s rotating during session",
        ("rt_k",),
        ("intraday_fast_quotes",),
        "secondary same-session quote confirmation",
        30,
    ),
    "minute_profile_capture": RuntimeTaskContract(
        "minute_profile_capture",
        "intraday_edge",
        "bounded market-session polling",
        ("intraday_minutes",),
        ("intraday_minute_sessions",),
        "minute features and time-of-day profile evidence",
        180,
    ),
    "longhu_order_book": RuntimeTaskContract(
        ...
        "3s during session",
        ("order_book_quote",),
        ("intraday_order_book_observations",),
        "bounded watchlist order-book capture",
        20,
    ),
    ...
    "strategy_review": RuntimeTaskContract(
        ...
        ("daily", "board_flow"),
        ("analyst_market_reviews", "strategy_reviews"),
        "post-close descriptive strategy review",
    ),
    "post_close_strategy": RuntimeTaskContract(
        "post_close_strategy",
        "research",
        "same-date post-close window",
        ("daily", "daily_basic"),
        ("strategy_candidates", "automation_runs"),
        "post-close research candidate generation",
    ),
    "ten_day_leader_rotation": RuntimeTaskContract(
        "ten_day_leader_rotation",
        "research",
        "scheduled post-close",
        ("daily", "limit_list_d"),
        ("ten_day_leader_rotation_runs",),
        "ten-day leader rotation research",
    ),
    "daily_strategy_summary": RuntimeTaskContract(
        ...
        "daily",
        (),
        ("strategy_day_summaries",),
        "research-only daily summary materialization",
    ),
    "ths_member_backfill": RuntimeTaskContract(
        ...
        "point-in-time THS constituent backfill",
    ),
    "all_board_member_backfill": RuntimeTaskContract(
        "all_board_member_backfill",
        "research",
        "bounded background batches",
        ("dc_member", "ths_member"),
        ("sector_membership_history",),
        "bounded board member coverage backfill",
    ),
    "retention_maintenance": RuntimeTaskContract(
        "retention_maintenance",
        "research",
        "daily after 16:30 CST",
        (),
        ("retention_policies",),
        "bounded batch delete for enabled retention policies",
    ),
}
...
def intraday_edge_task_labels() -> frozenset[str]:
    return frozenset(item.label for item in RUNTIME_TASK_CONTRACTS.values() if item.owner_profile == "intraday_edge")
