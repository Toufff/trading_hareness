...
_PRODUCTS = (
    ...
    _product("raw_market_observations", "raw", "available_at", ("provider_key", "capability", "exchange_date", "hour"), archive_format="jsonl_zstd", local_tier="cold_cache", local_hot_window_days=30, replay_role="source_audit", description="raw normalized and provider payload evidence"),
    _product("tushare_raw_records", "raw", "available_at", ("provider_key", "api_name", "available_date"), archive_format="jsonl_zstd", local_tier="cold_cache", local_hot_window_days=30, replay_role="source_audit", description="licensed provider row evidence with request identity"),
    ...
    _product("intraday_quote_observations", "canonical", "observed_at", ("exchange_date", "source_name", "symbol_bucket", "hour"), local_tier="hot", local_hot_window_days=30, replay_role="watch_quote_replay", description="watch-universe quote observations"),
    ...
    _product("intraday_advisory_events", "signals", "observed_at", ("exchange_date", "symbol_bucket"), local_tier="hot", local_hot_window_days=None, replay_role="account_advisory_replay", description="deterministic price and amount-pulse observations for holdings and formal recommendations"),
    _product("intraday_advisory_analysis_runs", "outcomes", "completed_at", ("provider", "completed_year"), local_tier="hot", local_hot_window_days=None, replay_role="account_advisory_replay", description="persisted DeepSeek and Codex advisory inputs and outputs"),
    _product("intraday_advisory_deliveries", "control", "created_at", ("delivery_kind", "created_year"), archive_format="jsonl_zstd", local_tier="hot", local_hot_window_days=None, replay_role="execution_audit_only", description="idempotent Feishu advisory delivery receipts"),
