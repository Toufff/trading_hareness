...
from ..event_read_model import market_announcements, market_events, market_lhb_events
from ..async_event_read_repository import market_announcements as async_market_announcements, market_events as async_market_events, market_lhb_events as async_market_lhb_events
...
def build_event_reads_router(database: Any, async_database: Any | None = None) -> APIRouter:
    ...
    @router.get("/api/v1/events/market")
    async def events(symbol: str | None = None, event_type: str | None = None,
                     trade_date: date | None = None, limit: int = 500, offset: int = 0) -> dict[str, Any]:
        return await async_market_events(async_database, symbol, event_type, trade_date, limit, offset) if async_database else market_events(database, symbol, event_type, trade_date, limit, offset)
