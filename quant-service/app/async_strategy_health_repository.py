...
async def latest_strategy_health(async_database: Any, *, now: datetime | None = None) -> dict[str, Any]:
    ...
    async with async_database.transaction() as connection:
        ...
        outcomes = await one(
            """SELECT count(*)::int AS rows, count(*) FILTER (WHERE raw_return > 0)::int AS positive, avg(raw_return) AS avg_return
                 FROM quant.intraday_signal_outcomes WHERE horizon_key='30m' AND status='matured' AND calculated_at >= now()-interval '7 days'"""
        )
