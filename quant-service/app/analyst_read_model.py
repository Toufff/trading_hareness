    ...
    with database.transaction() as connection:
        rows = connection.execute(
            """SELECT q.review_id,q.suggested_scope,q.suggested_label,q.suggested_symbol,q.direction,q.strength,q.horizon_days,
                      q.extraction_confidence,q.status,q.reviewer_note,q.created_at,e.remote_report_id,left(e.body,500) evidence,
                      a.name analyst_name
               ...
               JOIN quant.remote_analysts a ON a.remote_analyst_id=r.remote_analyst_id
