...
def compact_quote(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "cum_amount_million": _f(dec(row.get("cumulative_amount")) / 1_000_000, 1)
        if row.get("cumulative_amount")
        else None,
...
def summarize_minutes(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for index in range(0, len(done), 5):
        ...
        buckets.append(
            [
                chunk[0]["time"],
                chunk[-1]["close"],
                max(r["close"] for r in chunk),
                min(r["close"] for r in chunk),
    ...
    return {
        ...
        "low": min(prices),
        "last": last["close"],
        "high_time": done[prices.index(max(prices))]["time"],
        "low_time": done[prices.index(min(prices))]["time"],
        "vwap": round(last["vwap"], 3) if last.get("vwap") else None,
        "last_10_minutes": [
            [r["time"], r["close"], round((r.get("amount") or 0) / 1_000_000, 1), r["volume_lot"]] for r in done[-10:]
        ],
...
def compact_minutes(summary: dict[str, Any] | None) -> dict[str, Any] | None:
    """Keep decision-useful shape for non-held focus names without two full tapes."""
    if not summary or summary.get("status"):
        return summary
    scalar_keys = ("open", "high", "low", "last", "high_time", "low_time", "vwap", "note")
    return {
        **{key: summary.get(key) for key in scalar_keys},
        "last_5_minutes": (summary.get("last_10_minutes") or [])[-5:],
        "recent_5m_buckets": (summary.get("five_minute_buckets_time_close_high_low_amount") or [])[-6:],
    }


def summarize_daily_bars(rows: list[list[Any]] | None) -> dict[str, Any] | None:
    """Replace raw 20-row history with deterministic trend evidence for focus-only names."""
    if not rows:
        return None
    closes = [dec(row[4]) for row in rows if len(row) > 4 and dec(row[4]) > 0]
    lows = [dec(row[3]) for row in rows if len(row) > 3 and dec(row[3]) > 0]
    highs = [dec(row[2]) for row in rows if len(row) > 2 and dec(row[2]) > 0]
    amounts = [dec(row[6]) for row in rows[-5:] if len(row) > 6 and row[6] is not None]

    def change(sessions: int) -> float | None:
        if len(closes) <= sessions or closes[-sessions - 1] <= 0:
            return None
        return round(float((closes[-1] / closes[-sessions - 1] - 1) * 100), 2)

    return {
        "sessions": len(rows),
        "latest_mmdd_o_h_l_c_pct_amount_yi": rows[-1],
        "return_5d_pct": change(5),
        "return_10d_pct": change(10),
        "range_low_high": [_f(min(lows)) if lows else None, _f(max(highs)) if highs else None],
        "avg_amount_yi_last_5": _f(sum(amounts) / len(amounts), 1) if amounts else None,
    }


def context_part_chars(context: dict[str, Any]) -> dict[str, int]:
    """Stable per-key prompt-size telemetry, stored with model usage rather than sent to the model."""
    return {
        key: len(json.dumps(value, ensure_ascii=False, default=str, separators=(",", ":")))
...
def daily_bars(connection: Any, symbols: list[str], day: date, count: int = 20) -> dict[str, list[list[Any]]]:
    rows = _rows(
        ...
        SELECT symbol,trading_date,open,high,low,close,pre_close,amount FROM (
         ...
         WHERE rn <= %s ORDER BY symbol,trading_date""",
        (symbols, day, count),
    )
...
def board_flow(connection: Any, now: datetime, day: date) -> dict[str, Any]:
    ...
    output: dict[str, Any] = {
        "observed_at": row["observed_at"].astimezone(SHANGHAI).strftime("%H:%M:%S"),
        "net_inflow_unit": row["unit"],
    }
    for taxonomy, items in by_taxonomy.items():
        ...
        flow = sorted(
            [i for i in items if i.get("net_inflow") is not None], key=lambda i: i["net_inflow"], reverse=True
        )
        output[taxonomy] = {
            "count": len(items),
            "up_ratio": round(sum(1 for i in valid if i["change_pct"] > 0) / max(1, len(valid)), 2),
...
def breadth(connection: Any, now: datetime, day: date) -> dict[str, Any] | None:
    row = connection.execute(
        """SELECT source_snapshot_minute,market_state,concept_positive_ratio,concept_mean_change_pct,concept_median_flow,
            ...
            ORDER BY source_snapshot_minute DESC LIMIT 1""",
        (day, now),
    ).fetchone()
    if row is None:
        return None
    return {
        key: (
            ...
            else value.astimezone(SHANGHAI).strftime("%H:%M")
...
def rotations(connection: Any, now: datetime, day: date) -> list[list[Any]]:
    rows = _rows(
        ...
        SELECT snapshot_minute,label,taxonomy_key,event_type,direction,state,conditions->>'change_pct' AS chg,
         ...
         ORDER BY snapshot_minute DESC LIMIT 12""",
        (datetime.combine(day, datetime.min.time(), SHANGHAI), now),
    )
    return [
        [
            r["snapshot_minute"].astimezone(SHANGHAI).strftime("%H:%M"),
            r["label"],
            r["event_type"],
            r["direction"],
            r["state"],
...
def limit_events(connection: Any, now: datetime, day: date) -> dict[str, Any]:
    ...
    chains = _rows(
        ...
        SELECT DISTINCT ON (symbol) symbol,title FROM quant.market_events
         ...
         ORDER BY symbol,available_at DESC LIMIT 30""",
        (start, now),
    )
...
def signals(connection: Any, now: datetime, day: date) -> list[list[Any]]:
    ...
    return [
        [
            r["observed_at"].astimezone(SHANGHAI).strftime("%H:%M"),
            r["symbol"],
            r["signal_type"],
            r["state"],
            r["stage"],
...
def news(connection: Any, now: datetime) -> dict[str, Any] | None:
    row = connection.execute(
        """SELECT created_at,result FROM quant.event_research_runs
            WHERE status='analyzed' AND created_at<=%s ORDER BY created_at DESC LIMIT 1""",
        (now,),
    ).fetchone()
    if row is None:
        return None
    ...
    return {
        ...
        "events": [
            {"fact": e.get("fact"), "action": e.get("action"), "horizon": e.get("horizon")}
            for e in (result.get("events") or [])[:6]
        ],
        "leads": [
            [lead.get("title"), [s.get("symbol") for s in lead.get("symbols") or [] if isinstance(s, dict)]]
...
def plans(connection: Any, now: datetime) -> list[dict[str, Any]]:
    rows = _rows(
        ...
        SELECT DISTINCT ON (symbol) symbol,name,plan_kind,action,entry_zone,add_trigger,reduce_trigger,exit_trigger,
         ...
         WHERE created_at<=%s AND valid_until>=%s ORDER BY symbol,created_at DESC""",
        (now, now),
    )
    return [
        {
            **{k: v for k, v in row.items() if k not in {"created_at", "stop_price", "max_position_pct"}},
            "stop_price": _f(row["stop_price"]),
            "max_position_pct": _f(row["max_position_pct"], 1),
        }
        for row in rows
    ]
...
def candidates(connection: Any, now: datetime) -> list[list[Any]]:
    ...
    rows = _rows(
        ...
        SELECT rank,symbol,candidate_type,score,reason_codes,structure->'metrics' AS metrics,discovered_at
          FROM quant.post_close_strategy_candidates WHERE run_id=%s ORDER BY rank LIMIT 12""",
        (run["run_id"],),
    )
    return [
        [
            ...
            r["candidate_type"],
            _f(r["score"], 1),
            r["discovered_at"].astimezone(SHANGHAI).strftime("%m-%d"),
            {
                k: (r["metrics"] or {}).get(k)
                for k in ("support_price", "resistance_price", "close_to_resistance_pct", "sma20")
            },
        ]
        for r in rows
    ]
...
def recommendation_pool(connection: Any, now: datetime) -> dict[str, Any] | None:
    ...
    return {
        ...
        "reviewed_observe": [
            {k: item.get(k) for k in ("symbol", "name", "decision", "invalidation", "why_now")}
            for item in result.get("reviewed") or []
            if item.get("decision") != "recommend"
        ][:12],
        "strategy_screening_symbols": [
            [item.get("symbol"), item.get("name"), [m.get("lane") for m in item.get("memberships") or []]]
        ...
        lanes.append(
            {
                ...
                "top": [
                    [i.get("symbol"), i.get("name"), i.get("state"), i.get("price"), i.get("reason")] for i in items[:5]
                ],
            }
        )
...
async def build_context(
    ...
    *,
    ...
    with connection_factory() as connection:
        ...
        db_part = {
            "board_flow": board_flow(connection, now, day),
            "market_breadth": breadth(connection, now, day),
            "board_rotations": rotations(connection, now, day),
            "limit_up_events": limit_events(connection, now, day),
            "intraday_signals": signals(connection, now, day),
            "news_research": news(connection, now),
        }
    ...
    full_detail = list(dict.fromkeys(held + [o["symbol"] for o in open_orders] + [p["symbol"] for p in plan_rows]))
    full_detail_set = set(full_detail)
    compact_slots = min(FOCUS_DETAIL_LIMIT, max(0, DETAIL_LIMIT - len(full_detail)))
    compact_focus = [symbol for symbol in dict.fromkeys(focus) if symbol not in full_detail_set][:compact_slots]
    detail = full_detail + compact_focus
    pool_symbols = [
        item["symbol"] for item in (pool or {}).get("recommended", []) + (pool or {}).get("reviewed_observe", [])
    ]
    universe = list(
        dict.fromkeys(detail + pool_symbols + [row[0] for row in watch] + [row[1] for row in candidate_rows])
    )
    ...
    market_value = sum(
        (
            dec((quotes.get(row["symbol"]) or {}).get("price") or row["average_cost"]) * int(row["quantity"])
    ...
    context = {
        ...
        "account": {
            "cash": float(cash),
            "equity_live": round(float(equity), 2),
            "initial_equity": float(dec(account["initial_equity"])),
            "start_date": str(account["start_date"]),
            "positions": [
                {
                    "symbol": row["symbol"],
                    "name": row.get("name") or (quotes.get(row["symbol"]) or {}).get("name"),
                    "quantity": int(row["quantity"]),
                    "sellable": int(row["sellable_quantity"]),
                    "avg_cost": _f(row["average_cost"], 3),
                    "price": (quotes.get(row["symbol"]) or {}).get("price"),
                    "weight_pct": round(
                        float(
                            dec((quotes.get(row["symbol"]) or {}).get("price") or row["average_cost"])
                            * int(row["quantity"])
                            / equity
                            * 100
                        ),
            ...
            "open_orders": [
                {
                    ...
                    "placed_at": o["placed_at"].astimezone(SHANGHAI).strftime("%H:%M"),
                }
                for o in open_orders
            ],
            "today_orders": [
                [
                    o["placed_at"].astimezone(SHANGHAI).strftime("%H:%M:%S"),
                    o["symbol"],
                    o["side"],
                    o["order_type"],
                    o["quantity"],
        ...
        **db_part,
        "detail_symbols": {
            symbol: (
                {
                    "detail_level": "full_position_order_or_plan",
                    "quote": compact_quote(quotes[symbol]) if symbol in quotes else None,
                    "minutes_today": minutes.get(symbol),
                    "daily_last_20_mmdd_o_h_l_c_pct_amount_yi": daily.get(symbol),
                }
                if symbol in full_detail_set
                else {
                    "detail_level": "compact_focus",
                    "quote": compact_quote(quotes[symbol]) if symbol in quotes else None,
                    "minutes_today": compact_minutes(minutes.get(symbol)),
                    "daily_summary": summarize_daily_bars(daily.get(symbol)),
                }
            )
            for symbol in detail
        },
        "watchlist_quotes_symbol_name_price_pct": [
            [
                s,
                (quotes.get(s) or {}).get("name") or label,
