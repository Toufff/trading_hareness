...
def build_specs(
    *, interval_seconds: int, enabled: dict[str, bool], loops: dict[str, Callable[[], Any]]
    ...
    return tuple(
        BackgroundTaskSpec(label, bool(enabled.get(label)), loops[label]) for label in labels if label in loops
    )
