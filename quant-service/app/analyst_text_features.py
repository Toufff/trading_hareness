...
def summary_from_rows(
    reports: list[dict[str, Any]], topic_claims: list[dict[str, Any]], as_of_date: date,
    *, classify_text: Callable[[str], tuple[int, float, float]], factor_version: str, lookback_days: int,
) -> dict[str, Any]:
    """Aggregate prefetched point-in-time rows without a database dependency."""
...
def analyst_text_factor_summary(connection: Any, as_of_date: date, *, classify_text: Callable[[str], tuple[int, float, float]],
    ...
    return summary_from_rows(
        reports, topic_claims, as_of_date,
