...
- 特征与方向推荐：日线趋势、成交量、东财主力/散户资金流、基础估值和已审核分析师观点共同评分；推荐保存特征快照、置信度、风险标记、失效条件和有效期；特征快照读到 as_of 当日已结算（`available_at` 不早于当日 15:05 且不晚于观察时点）的收盘，结果归因在下一交易日开盘入场（2026-09-19 决定，见 `docs/POST_CLOSE_RESEARCH_CLOSURE.md` 修改 E）；
