...
# The release the shared-peer tunnel is really executing out of. After a run of
# skipped publishes it is neither the active nor the previous release, which is
# exactly why retention pins it -- and why an operator needs to see it before
# deciding a release directory is safe to remove.
...
$releases = if (Test-Path -LiteralPath $layout.ReleasesRoot -PathType Container) {
    @(Get-ChildItem -LiteralPath $layout.ReleasesRoot -Directory -Force | Where-Object Name -notlike '.staging-*' | Sort-Object LastWriteTimeUtc -Descending | ForEach-Object {
        ...
        [ordered]@{
            ...
            shared_peer_tunnel = [bool]($tunnelRelease -and $tunnelRelease -eq $_.Name)
