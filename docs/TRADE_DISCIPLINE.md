...
状态：设计已定，实施中（2026-09-18）；第二轮审查（600613 纪律卡）后于同日修订，见文末“迭代记录”。本文件是实现的唯一合同；实现与本文冲突时先改本文再改代码。
...
class DisciplinePlan(BaseModel):
    ...
    metrics: dict[str, Any]           # 生成时冻结的指标快照（ma5/ma10/ma20/atr14/hi20/lo20/low20/recent_low/prior_high/drawdown/...）
                                      # 另含 omitted_lines（规则拒绝的线）与 t1_locked_shares（生成日不可卖股数 = quantity − sellable_quantity）
    sizing: Sizing | None
...
- `hard_stop`（必有，priority 1）：`metric=daily_close, op="<", confirm(bars=1, daily)`，价格 = `min(structure_low, reference_price × (1 − buffer), reference_price − 0.9 × ATR14, reference_price × (1 − 2%))`，四项全部以 inputs 记入 derivation（`buffer_pct`、`atr_target_multiple=0.9`、`stop_pct_target=0.02`），formula 逐项写出——即结构点只允许**下移**到最小距离，永远不许被抬到结构点之上；结构比 3×ATR14 / 12% 还远时价格保持不动，由 `hard_stop_distance_sane` 判不过、按原则 9 落库为 `rejected_by_quality`。其中 structure_low 按 stage 取：crash_rebound 用 `low20`（最近 20 个交易日的最低价，即急跌低点）；broken/unclassified 用 `min(当日低点, 昨日低点)`；pullback 用 `recent_low`（近 5 日收盘低）；trend 用 `MA10 × (1 − 0.5%)`；breakout 用 `突破平台 prior_high × (1 − 0.5%)`；base_platform 用 10 日最低收盘。buffer = `max(1.5%, 0.6 × 日波动率%)`（与 risk.py 一致）。另给一个盘中副本 `metric=minute_close, confirm(bars=3, minute)`，同价，label 标“盘中版”。
- `soft_stop`（可选，priority 2）：价格 = MA5，允许 extra 条件，action=reduce_by_pct 50。**仅当** `hard_stop + 0.5 × ATR14 <= MA5 <= reference_price − 0.5 × ATR14` 时生成；否则不生成，并把 `{kind:"soft_stop", reason, inputs:{ma5, hard_stop, reference_price, atr14, window_low, window_high}}` 写入 `metrics.omitted_lines`。理由：软止损离现价不足半个 ATR 时下一日噪音即触发，离硬止损不足半个 ATR 时与硬止损无差别。
...
- `holiday`（valid_until 内存在 ≥5 自然日休市时必有；3 天的节日长周末不出线）：最后交易日收盘前把仓位降到 `holiday_exposure_pct` = 阶段目标仓位的一半（crash_rebound 10、broken 0、breakout_hold 12.5、trend_hold 15、pullback_hold 15、base_platform 10、unclassified 7.5）。休市缺口只能由**真实开市日**起算：生成日若本身闭市，不得作为 `last_trading_date`。
...
- `exposure_line_when_over_cap`、`holiday_line_when_closure`（有效期内存在 ≥5 自然日休市时必须有 holiday 线；从冻结的 `metrics.closure.closed_days` 重新判定，不只信生成器的标记）、`no_add_when_crash_or_broken`
...
- `report.py`：为每个计划生成 Markdown 纪律卡与 JSON：头部（股票、阶段、持仓、有效期、状态），仓位表（equity、单笔风险比例与其并排的 `current_risk_pct`、风险预算、止损距离、max/recommended shares、当前持仓股数、`sellable_quantity`、仓位比例按参考价与按快照市值 `market_value / equity` 两行），仓位表下当 `sellable < quantity` 时一句“生成日不可卖 N 股（T+1），价格线自下一交易日起可执行”，线表（kind、条件人话、价格、执行方式、优先级、rule_id、formula），线表后的“未生成的线及原因”（来自 `metrics.omitted_lines`），推导表（价格与动作值各自复算），质量门结果，证据引用。写到 `<output_root>/<trading_date>/<symbol>-<plan_key>.md/.json`，默认 `G:/StockPlatform/reports/discipline/`。
- `inputs.py` 的日线口径：`as_of` 在 15:00 及之后且 `canonical_bars_daily` 已有当日已结算 bar 时，直接用已结算 bar、不读实时行情，`evidence_refs` 记 `bars_basis:settled`；只有当日 bar 缺席才允许用 live_quote + 分钟线合成 forming bar，记 `bars_basis:settled_plus_forming` 与 `forming_bar:<source>:<date>`；两者都没有记 `bars_basis:settled_only`。
