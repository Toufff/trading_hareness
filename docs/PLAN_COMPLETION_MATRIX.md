...
| `main.py` 完全拆分 | 工程余项 | router/read-model/纯规则已拆出；盘中侧现进一步分离了观察池报价采集、扫描总编排、单事务信号持久化、全板块成员批次与 THS 概念成员恢复。容量/覆盖度/就绪度、单股窗口/claim 证据、特征读取、分析师文本、日线/板块同步、盘后编排、盘口/分钟/板块曲线及远端文字同步也均有独立模块。主文件当前约 5,540 行；仍保留明确的 composition root、兼容入口和少量写服务，旧兼容快照待获授权的回放验证后再删除。 |
