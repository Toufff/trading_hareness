...
| 实时报价来源与时间戳门禁 | 已完成 | 同轮腾讯观察池批量报价才可确认，重点窗口须在 20 秒、其他窗口 45 秒内；新浪/腾讯全 A 快照严格标为证据来源并按真实 provider 落库，不可直接推送。进入个股条件的腾讯全 A 资金流带快照年龄，超过 45 秒不得确认新的 entry；`/intraday/services/status` 展示可确认覆盖、回退数，覆盖不足时将决策路径标为 degraded；普通时段的 Super GET `rt_min` 每轮至多验证 4 只，但游标按实际观察池长度闭环推进，36 只池在 9 轮内全部覆盖 |
...
| `main.py` 完全拆分 | 工程余项 | router/read-model/纯规则已拆出；容量/覆盖度/就绪度投影、特征读取、特征快照物化、分析师文本聚合与 scorecard 重算、推荐生成、Tushare/BaoStock 日线同步、全市场股票池/全市场日线同步与归一化、THS 六类板块目录编排及单类成员同步、THS 概念成员分页、东方财富板块成员同步、实时资金流板块补水、THS 行业/概念资金流物化、盘后 outcome 重算、**盘中调度、秒级交叉确认及 Super GET 分钟上下文归一化/扇出**、分钟特征、盘中信号规则、盘中 outcome 归因与结算、远端分析师 transport/差量同步编排、盘后一键刷新/日流水线编排、盘后模式评分、候选筛选与**候选写服务**、涨停/连板样本选择、盘后板块/LHB证据聚合、涨停模式读模型、涨停分钟形态、盘中归因标签、突破评估/确认、涨停日特征、观察池日因子和竞价时段整理已迁至独立模块；盘中/推荐/Tushare/BaoStock/股票池/全市场日线/THS/东方财富/outcome 旧实现暂保留为未调用兼容快照，待回放验证后再删除。仍有写服务和策略编排兼容函数待机械迁移 |
...
- quant-service：405 项 Python 测试通过（包含盘中连续竞价结算、分析师 received-at 与 author-stated 双时钟、零项同步 liveness 回执、事件生命周期回放确定性与幂等、策略族健康聚合、JSON 数值归一化、时间外金标留出回归、研究/热库硬上限与实际观察池长度的 `rt_min` 轮转覆盖）。
...
- 分析师报告/消息工作流已经拆分、凭据域名和 JSON Body 已修复；本轮又将 `workflow_entity.versionId` 与发布版本对齐并重启 n8n。2026-08-16 已从 n8n 容器用现有凭据对报告/消息流各做一次等价的有界直接请求，均返回 HTTP 200。零项消息轮询也会记录独立的 45 日 liveness 回执，而不伪造内容游标推进；健康页因此能准确给出“服务可达、等待当前发布版工作流执行”的状态。外置 runner 曾出现定时触发后未领取 HTTP 节点任务，旧执行只保留为审计并由 `scripts/reconcile-stale-n8n-executions.sh` 收口；当前仍需一次“当前发布版本 + success”的正式运行证据，故 P0-A1 不标为完全验收。
