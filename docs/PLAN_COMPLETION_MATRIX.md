...
| 原生 async repository | 部分完成 | 策略决策/复盘/盘后候选、策略健康、策略消融、纸面研究、事件/龙虎榜、涨停/连板模式、研究目录，以及市场快照/原始 Tushare/分钟导入/最新推荐/指标计数、研究总览、**Agent 自动化回执、市场资金流特征、全部板块/概念/精确成员证据和涨停联动候选**、分析师成绩单和研究就绪度（含历史容量估算与 replay readiness）、观察池列表、最新扫描/投递证据、决策卡、板块分钟曲线/收盘报告/轮动/挖掘候选及分析师操作回放/outcome、技能/研究/归档/观点/审核账本等 GET 已使用 `AsyncDatabase`；其余读写仓储仍经有界同步执行器，健康页显示异步池水位 |
...
- quant-service：当前构建后的全量 **658** 项 Python 回归通过。除历史的 v2 规则输入、policy/risk gate 与作者时点回放边界外，新增覆盖观察池报价的直连/兜底边界、扫描总编排的闭市零外呼和确认投递、单事务信号证据顺序、THS/东财精确成员批次失败隔离、概念目录不可用时成员恢复 fail-closed、概念/涨停池精确代码 join，以及原生异步自动化回执、市场资金流、板块/概念/精确成员、涨停联动、决策卡、板块曲线/复盘/轮动/挖掘、分析师操作回放/outcome、技能/研究/归档证据读取。
