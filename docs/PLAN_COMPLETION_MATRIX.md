...
| 原生 async repository | 部分完成 | 策略决策/复盘/盘后候选、策略健康、策略消融、纸面研究、事件/龙虎榜、涨停/连板模式、研究目录，以及市场快照/原始 Tushare/分钟导入/最新推荐/指标计数、研究总览、分析师成绩单和研究就绪度（含 replay readiness）GET 等只读投影已使用 `AsyncDatabase`；历史容量估算仍经有界数据库执行器运行，其他读写仓储仍经有界同步执行器，健康页显示异步池水位 |
| `main.py` 完全拆分 | 工程余项 | router/read-model/纯规则已拆出；容量/覆盖度/就绪度投影、特征读取、特征快照物化、分析师文本聚合、数值解析、盘中时钟、分钟特征、涨停日特征、观察池日因子和竞价时段整理已迁至独立模块，旧分析师聚合实现已删除，当前主文件约 9,589 行，仍有大量写服务和策略编排兼容函数待机械迁移 |
...
- quant-service：310 项 Python 测试通过（含事件/龙虎榜与涨停/连板模式异步只读路由、容量/特征读取、数值/时钟/分钟及涨停日特征模块和分析师文本 PIT 回归）。
...
- 最近提交：`c000ec2`（回放就绪度原生异步只读仓储）；当前未改变策略阈值或历史数据范围，竞价时段整理和回放门禁已通过 310 项回归。
