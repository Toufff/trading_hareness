...
| `main.py` 完全拆分 | 工程余项 | router/read-model/纯规则已拆出；容量/覆盖度/就绪度投影、特征读取、分析师文本聚合、数值解析、盘中时钟、分钟特征和涨停日特征已迁至独立模块，旧分析师聚合实现已删除，当前主文件约 9,725 行，仍有大量写服务和策略编排兼容函数待机械迁移 |
...
- 最近提交：`7215f46`（分钟特征拆分）；当前未改变策略阈值或历史数据范围，涨停日特征拆分已通过 308 项回归，待本轮提交后推送。
