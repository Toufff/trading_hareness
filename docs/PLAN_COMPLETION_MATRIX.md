...
| `main.py` 完全拆分 | 工程余项 | router/read-model/纯规则已拆出；容量/覆盖度/就绪度投影、特征读取和分析师文本聚合已迁至 `app/research_capacity.py`、`app/feature_read_repository.py`、`app/analyst_text_features.py`，当前主文件约 9,975 行（仍含待删除的旧分析师聚合回滚实现），仍有大量写服务和策略编排兼容函数待机械迁移 |
...
- quant-service：304 项 Python 测试通过（含事件/龙虎榜与涨停/连板模式异步只读路由、容量/特征读取和分析师文本 PIT 回归）。
...
- 最近提交：`a23a91c`（研究只读边界回归）；本轮未改变策略阈值或历史数据范围，已推送到 `origin/main`。
