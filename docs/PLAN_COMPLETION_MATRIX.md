...
| `main.py` 完全拆分 | 工程余项 | router/read-model/纯规则已拆出；容量/覆盖度/就绪度投影、特征读取、分析师文本聚合和数值解析已迁至独立模块，旧分析师聚合实现已删除，当前主文件约 9,868 行，仍有大量写服务和策略编排兼容函数待机械迁移 |
...
- quant-service：305 项 Python 测试通过（含事件/龙虎榜与涨停/连板模式异步只读路由、容量/特征读取、数值模块和分析师文本 PIT 回归）。
...
- 最近提交：`8049208`（删除旧分析师聚合实现）；当前未改变策略阈值或历史数据范围，数值解析拆分已通过 305 项回归，待本轮提交后推送。
