...
| `main.py` 完全拆分 | 工程余项 | router/read-model/纯规则已拆出；容量/覆盖度/就绪度投影、特征读取和分析师文本聚合已迁至 `app/research_capacity.py`、`app/feature_read_repository.py`、`app/analyst_text_features.py`，旧分析师聚合实现已删除，当前主文件约 9,872 行，仍有大量写服务和策略编排兼容函数待机械迁移 |
...
- 最近提交：`a23a91c`（研究只读边界回归）；当前未改变策略阈值或历史数据范围，待本轮删除死代码的提交验证后推送。
