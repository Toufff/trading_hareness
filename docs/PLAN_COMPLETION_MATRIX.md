...
| `main.py` 完全拆分 | 工程余项 | router/read-model/纯规则已拆出；容量/覆盖度/就绪度投影、特征读取、特征快照物化、分析师文本聚合、数值解析、盘中时钟、分钟特征、涨停分钟形态、盘中归因标签、突破评估/确认、涨停日特征、观察池日因子和竞价时段整理已迁至独立模块，旧分析师聚合实现已删除，当前主文件约 9,223 行，仍有大量写服务和策略编排兼容函数待机械迁移 |
...
- quant-service：311 项 Python 测试通过（含事件/龙虎榜与涨停/连板模式异步只读路由、研究就绪度原生异步查询、容量/特征读取、数值/时钟/分钟、盘中归因、突破评估/确认及涨停日特征模块和分析师文本 PIT 回归）。
