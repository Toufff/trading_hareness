...
| `main.py` 完全拆分 | 工程余项 | router/read-model/纯规则已拆出；容量/覆盖度/就绪度投影、特征读取、分析师文本聚合、数值解析和盘中时钟已迁至独立模块，旧分析师聚合实现已删除，当前主文件约 9,848 行，仍有大量写服务和策略编排兼容函数待机械迁移 |
...
- quant-service：306 项 Python 测试通过（含事件/龙虎榜与涨停/连板模式异步只读路由、容量/特征读取、数值/时钟模块和分析师文本 PIT 回归）。
...
- 最近提交：`8049208`（删除旧分析师聚合实现）；当前未改变策略阈值或历史数据范围，数值/时钟拆分已通过 306 项回归，待本轮提交后推送。
