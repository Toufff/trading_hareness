...
| 原生 async repository | 部分完成 | 策略决策/复盘/盘后候选、策略健康、策略消融、纸面研究、事件/龙虎榜、涨停/连板模式、研究目录，以及市场快照/原始 Tushare/分钟导入/最新推荐/指标计数等只读投影已使用 `AsyncDatabase`；研究总览和分析师成绩单仍含同步回调，其他读写仓储仍经有界同步执行器，健康页显示异步池水位 |
