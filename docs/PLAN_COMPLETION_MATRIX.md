...
- quant-service：308 项 Python 测试通过（含事件/龙虎榜与涨停/连板模式异步只读路由、容量/特征读取、数值/时钟/分钟及涨停日特征模块和分析师文本 PIT 回归）。
...
- 最近提交：`9cb12f0`（涨停日特征拆分）；当前未改变策略阈值或历史数据范围，竞价时段整理已通过 308 项回归，待本轮提交后推送。
