...
| 实时报价来源与时间戳门禁 | 已完成 | 同轮腾讯观察池批量报价才可确认，重点窗口须在 20 秒、其他窗口 45 秒内；新浪/腾讯全 A 快照严格标为证据来源并按真实 provider 落库，不可直接推送；`/intraday/services/status` 展示可确认覆盖、回退数，覆盖不足时将决策路径标为 degraded |
