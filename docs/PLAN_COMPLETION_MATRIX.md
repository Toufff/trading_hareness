...
| `main.py` 完全拆分 | 工程余项 | router/read-model/纯规则已拆出；容量/覆盖度/就绪度投影、特征读取、特征快照物化、分析师文本聚合、Tushare 归一化、盘中时钟、分钟特征、盘中信号规则、盘中 outcome 归因与结算、远端分析师 transport/差量同步编排、盘后模式评分、盘后候选筛选、涨停/连板样本选择、盘后板块/LHB证据聚合、涨停模式读模型、涨停分钟形态、盘中归因标签、突破评估/确认、涨停日特征、观察池日因子和竞价时段整理已迁至独立模块；盘中结算旧实现暂保留为未调用的兼容快照，待历史回放验证后再删除。仍有大量写服务和策略编排兼容函数待机械迁移 |
...
- quant-service：317 项 Python 测试通过（含事件/龙虎榜与涨停/连板模式异步只读路由、研究就绪度原生异步查询、容量/特征读取、Tushare 归一化、盘后候选筛选、板块/LHB证据聚合、涨停模式读模型、数值/时钟/分钟、盘中信号规则、盘中 outcome 归因与结算委托、分析师同步健康的当前发布版本校验、远端文本 transport/差量同步的 Retry-After/限速、盘后模式评分、盘中归因、突破评估/确认模块与兼容入口等价性以及涨停日特征模块和分析师文本 PIT 回归）。
...
- 最近提交：见当前仓库最新提交；本轮未改变策略阈值或历史数据范围，盘中归因/规则/结算拆分、分析师同步健康校验、远端文本 transport 拆分、盘后模式评分/候选筛选/证据聚合/读模型委托、竞价时段整理和研究就绪度门禁已通过 317 项回归。
