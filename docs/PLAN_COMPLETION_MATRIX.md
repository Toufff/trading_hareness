...
| 实时报价来源与时间戳门禁 | 已完成 | 同轮腾讯观察池批量报价才可确认，重点窗口须在 20 秒、其他窗口 45 秒内；新浪/腾讯全 A 快照严格标为证据来源并按真实 provider 落库，不可直接推送；`/intraday/services/status` 展示可确认覆盖、回退数，覆盖不足时将决策路径标为 degraded；普通时段的 Super GET `rt_min` 每轮至多验证 4 只，但游标按实际观察池长度闭环推进，36 只池在 9 轮内全部覆盖 |
