...
- 九策略均执行；高级形态依赖真实日K。公平补充最多96只代表候选的80日OHLC，分钟至少补当前各策略前排和原展示名单。不是全市场逐票分钟/80日K扫描，未补对象保留缺口，不能当作不合格。
...
- 午盘结果必须带`presentation`确定性摘要。昨日正式推荐从`recommendation_pool_decisions`读取并按正式`priority`单列；历史候选、新发现和当前前排只在各自策略内按状态、正式排名、成交额和代码排序。不同策略分数不可混成总分，Agent不得再从混合表临时挑股。
