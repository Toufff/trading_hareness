...
## 2026-09-22 通知降噪 v2（当前契约）

以 [INTRADAY_NOTIFICATIONS_V2.md](INTRADAY_NOTIFICATIONS_V2.md) 为当前通知契约。
仅 10:00 / 11:35 / 14:45 三次阶段简报；普通异动不再触发模型跟发；
十分钟检查仅解释真实计划条件更新；成功自检静默，持续故障及已通知故障恢复才通知。

## 2026-09-22 增量卡片契约（覆盖下文旧版描述）

- 实施与验收见 [INTRADAY_PRESSURE_V1.md](INTRADAY_PRESSURE_V1.md)。
- 个股确定性卡使用 `pressure-v1`：同一交易所时钟的 1/3/5 分钟价格、成交额和内外盘增量；
  合并同股多窗口事件。第一屏显示区间、涨跌、放缩量和描述性多空状态。
- ten_minute 使用条件更新专用输入、输出和展示结构；没有真实条件更新则静默。
  fixed/midday/tail 为三次阶段关键事项简报。独立纪律告警链路不变。
- 委比仅为完整五档的辅助挂单事实，不参与方向判断，不作为独立触发，不推断撤单或机构身份。
- 模型运行在单个后台任务，不阻塞 5 秒采样；不能并发堆积模型调用。过期增量解释不推送。
- 所有代码名称与持仓/候选角色绑定校验；候选禁止收到减仓卖出指令。无百分制多空分数。
...
| 条件更新检查 | 盘中十分钟时点，阶段简报优先 | 有真实条件变化才调用 DeepSeek | 无变化不调用、不推送；身份与原条件绑定校验 |
| 阶段简报 | 10:00 / 11:35 / 14:45 | Codex | 当日每类一条；数据不足跳过，持久化防重 |
| 突发或纪律线补充 | 已停用自动跟发 | 否 | 确定性行情/纪律卡独立发送，不再自动追加模型解释 |
...
- 正常和休市静默；自愈后仍失败才通知业务影响，故障去重，已通知故障验证恢复才再通知；
