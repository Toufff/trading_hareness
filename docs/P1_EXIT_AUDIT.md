...
| HTTP 调用所有权 | 策略、router、仓储与读模型不得绕过生命周期 HTTP 池；公共源、Tushare 与飞书各走独立受控 client | 静态 transport-ownership 回归；当前构建全量 Python discovery 728 项通过 |
| Prometheus 控制面 | `/metrics` 自行以 5 秒 TTL 刷新本地连接池水位和当前 provider 熔断数；数据库异常不使 scrape 失败 | metrics-control-plane 回归；当前构建全量 Python discovery 728 项通过 |
...
3. **P4 组合层**：纸面账户、人工接受、T+1/费用/涨跌停与停牌门禁、目标仓位映射、日亏/回撤/板块集中度阻断已落地；严格意义的组合级自动熔断、策略健康度自动降级仍待 P3 样本门禁通过后启用。
...
- 新增迁移后应立即生成一次可恢复的本地归档；截至本次审计，最近校验的可恢复归档为 `20260823-190003-daily`，含 14 份 workflow JSON，`pg_restore -l` manifest 与归档权限均已通过开盘预检。运行时如健康页缺少任一背景 lease，必须先恢复/验证续租再宣称开盘就绪。
