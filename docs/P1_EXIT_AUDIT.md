...
更新时间：2026-08-11。此文档是 P1 当前可验证状态的交接清单，不把 P2 的历史回填、分钟回放或策略阈值校准提前宣称为完成。
...
| 盘中扫描 | 腾讯失败、空池、闭市门禁、正常完成均写可审计终态；扫描总耗时按 outcome 暴露 | `quant_intraday_scan_duration_seconds`；闭市真实调用为 `blocked` |
...
3. **P4 组合层**：纸面组合、仓位映射、组合级回撤熔断和策略健康度自动降级；只能消费已通过 P3 门禁的规则。
