...
- `EQUITY_DAILY_CONTROL_STATUS_SQL` 改为按 `exchange`（symbol 后缀 SH/SZ/BJ）分组统计 expected 与 daily/adjustment/limit 行数，同时保留总行。
- `status_payload` 新增：`by_exchange: {SH:{expected,daily,ratio}, SZ:{...}, BJ:{...}}`；`gating_exchanges: ["SH","SZ"]`（常量 `GATED_EXCHANGES`，北交所在有日线源前不参与门槛）；`coverage_ratio`、`minimum_required_rows`、`state` 只按 gating exchanges 计算；`ungated_exchanges` 列出未参与门槛的交易所及其覆盖，供报告展示。
- 新增 `expected_previous_trading_day` 与 `expected_delta`（与上一交易日的 all_a 预期数差值，按 `quant.universe_membership_history` 用 `effective_from/effective_to` 回算）；`|expected_delta| > 2% × expected` 时 `reason` 里必须写明“预期宇宙数变化 +N（来源分组：source→count）”，来源分组取 `effective_from = 当日` 的行按 `source` 计数。
- `reason` 文案在 blocked 时必须同时给出分子、分母、按交易所的覆盖和 delta，例如 “SH+SZ 5122/5221=98.1% ready；BJ 0/342 未参与门槛；all_a 预期较上一交易日 +305（stock-basic-all-a:tushare_super_get 312, longhuvip_composite 12）”。
...
- 任何读取 `pipeline_status`/`status` 的下游（`get-stock-release-status.ps1`、复盘页导出、飞书投递）若对 `completed` 做了等值判断，改为同时接受 `completed_research_due`；用 grep 找全，不能漏。
