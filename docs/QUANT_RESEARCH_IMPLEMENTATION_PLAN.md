...
状态：本地基础链路已实施并验收；真实行情历史回填、provider 授权和 Qlib 实验等待合规数据源配置  
...
- 只有 Tushare 单源、逐标的串行日线同步；没有历史回填、批量调用、重试、熔断和 provider 健康状态。
- `quant.market_bars_daily` 以 `(symbol, trading_date)` 为主键，第二来源会覆盖第一来源，无法保留多源证据和差异。
- 当前 `mean_excess_return` 实际是标的原始收益，没有扣除沪深 300 或行业基准，命名与实现不一致。
- 远端报告的规范化目前仅可靠识别显式六位代码和少量情绪词；公司简称、行业/主题、条件句、目标价、风险线和观点变化仍需要证券主数据映射与复核。
- 当前评分仅使用分析师共识、简化命中率和 5/20 日价格动量；缺少成交量、流动性、波动率、行业强度、公告风险、拥挤度和数据质量惩罚。
- 16:45 调度早于部分盘后数据稳定时间，也没有交易日历、新鲜度门禁和分阶段重跑。
- 没有严格的 point-in-time 数据集、A 股 T+1/涨跌停/停牌/费用规则、walk-forward 回测和模型版本比较。
- Vue 页面目前只有投递与采集监控，没有数据源诊断、分析师画像、候选证据链和历史表现页面。
