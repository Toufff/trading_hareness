...
- 全量 279 项测试通过，覆盖上海日期、四种涨跌停规则、复权除权日收益无伪跳空、涨停形态按 20%/30% 缩放、年度日历请求、网关错误脱敏、腾讯前复权隔离，以及真实 PostgreSQL 的 `upsert_bar` 三态 SQL 保护和盘中 episode 链接修复。
- 手工备份与 LaunchAgent 首次备份均通过 `pg_restore -l` 校验；工作流导出 12 份。
- 认证上线后，缺失 key 的写请求返回 HTTP 401；新增 ASGI `TestClient` 回归覆盖实际服务应用：无 key/错 key 均为 401，正确 key 可抵达受保护路由并进入正常的 422 参数校验。服务重建后全量 279 项测试全部通过；数据库池当前为 4 个已建立连接、0 个等待，最大上限 12。
