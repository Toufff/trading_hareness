...
- 行情、持仓/推荐范围和纪律线由确定性 watcher 每 30 秒计算。分钟/日线价格条件按各自确认口径触发；到期复核、时间止损等时间线在截止时点触发，并与价格线共用幂等 outbox。当前 watcher 固定绑定 `citics-primary`；Codex 负责健康巡视、失败诊断和解释，不用大模型逐分钟猜测价格，也不提交订单。
