# macOS + Colima era (archived)

**Status: archived, 2026-09-02.** Everything below describes the original
local deployment of this project: a single macOS workstation, Docker provided
by Colima (not Docker Desktop), and a checkout that lived at
`/Users/papa/codebase/n8n` (hence the `n8n/scripts/...` paths below — that is
a directory name from the old checkout, not a subdirectory of this
repository). Production has since moved to the Windows `G:\StockPlatform`
platform described in [`../AGENT_HANDOFF.md`](../AGENT_HANDOFF.md) and
[`../SHARED_PEER_RUNTIME.md`](../SHARED_PEER_RUNTIME.md); `README.md`,
`OPERATIONS.md` and `DEPLOYMENT.md` at the repository root now point here
instead of describing this workflow as current.

Do not follow these instructions against the current Windows deployment: the
paths, container names, `launchctl`/`colima` commands and LaunchAgents below
do not exist there. The scripts and `.plist` files this page references were
moved verbatim to [`legacy/macos/`](../../legacy/macos/) (see the WP3 work
package report) so their content remains available for reference; a few
specific paths are annotated below where they moved.

The Feishu relay/workbench *behavior* described here (webhooks, media
chunking, group relay, workbench actions) was largely carried forward — only
how the adapter process is started and supervised changed (it now runs
directly via `node.exe` under the Windows runtime supervisor, not inside a
Colima-managed Docker Compose stack; see `scripts/windows/start-stock-dashboard.ps1`).
If any of the macOS-only companion tooling below (the LLDB-based WeChat text
key capture, the WeChat image watcher, the global relay hotkey) is still in
use on a separate macOS companion machine, verify its current environment
against this page before trusting it as authoritative — it has not been
re-verified as part of the Windows migration.

---

## Original README: "Local n8n (Colima)"
...
## Original OPERATIONS.md: "本地 n8n + 飞书机器人运行手册"

最后确认：2026-08-08（归档前的最后一次确认）。本目录是一套仅绑定本机回环地址的飞书机器人自动化服务。它使用 Colima 提供 Docker，不依赖 Docker Desktop。
...
在任意桌面应用中复制文字后，可运行 `scripts/relay-from-clipboard.sh`。脚本会把文字放入只保留 5 分钟的一次性本机草稿，并自动打开投递台、填好正文；剪贴板内容不会出现在 URL 或浏览器历史中。图片仍可在投递台内直接粘贴。（这个脚本没有随其余 macOS 专属脚本移动，仍在 `scripts/relay-from-clipboard.sh`，因为它只是打开本机浏览器 URL，与 launchd/Colima 无关。）

已安装的本机 LaunchAgent `com.papa.market-relay-hotkey`（这个 `.plist` 从未提交进本仓库，是操作员本机独立配置的全局快捷键注册项）会在登录后自动注册全局快捷键 `⌃⌥⌘R`，不需要再在"快捷指令"中配置。该服务只会在按下快捷键时读取一次剪贴板；不会持续监控、保存或上传剪贴板内容。若"快捷指令"里留有刚才新建的空白快捷指令，可直接关闭且不保存。快捷键背后调用的签名/权限修复脚本已随其余 macOS 遗留脚本移动到 [`legacy/macos/scripts/relay-hotkey.swift`](../../legacy/macos/scripts/relay-hotkey.swift)、[`legacy/macos/scripts/setup-relay-codesign.sh`](../../legacy/macos/scripts/setup-relay-codesign.sh)、[`legacy/macos/scripts/fix-relay-fda.sh`](../../legacy/macos/scripts/fix-relay-fda.sh)。

#### 微信本机图片监控

如果只需要转发 Mac 微信里指定群的本机图片/音频/视频，可以启动本机 watcher。它只读取本机微信媒体临时目录中的新增媒体文件，不读取微信数据库、Cookie、网络流量或聊天文本；启动前已有的历史文件会被忽略。这个 watcher 脚本已随其余 macOS 遗留脚本移动到
[`legacy/macos/scripts/wechat-image-relay.py`](../../legacy/macos/scripts/wechat-image-relay.py)、
[`legacy/macos/scripts/start-wechat-image-relay.sh`](../../legacy/macos/scripts/start-wechat-image-relay.sh)、
[`legacy/macos/scripts/stop-wechat-image-relay.sh`](../../legacy/macos/scripts/stop-wechat-image-relay.sh)（原文档中的
`n8n/scripts/...` 前缀是旧 checkout 目录名，不是本仓库路径）。
...
```bash
legacy/macos/scripts/start-wechat-image-relay.sh xiaolan
legacy/macos/scripts/start-wechat-image-relay.sh liuzi
legacy/macos/scripts/start-wechat-image-relay.sh liwei
```

停止服务：

```bash
legacy/macos/scripts/stop-wechat-image-relay.sh
```

日志与去重状态（路径同样是旧 checkout 目录名下的相对路径，不是本仓库路径）：
...
当前机器还安装了用户级 LaunchAgent（`.plist` 内容已归档在
[`legacy/macos/launchd/com.papa.wechat-image-relay.plist`](../../legacy/macos/launchd/com.papa.wechat-image-relay.plist)）：
...
```bash
WECHAT_RELAY_CHAT_DIR_ID=<目录ID> legacy/macos/scripts/start-wechat-image-relay.sh xiaolan
```
...
`scripts/wechat-text-relay.py`（连同 `scripts/start-wechat-text-relay.sh`/`stop-wechat-text-relay.sh`）与其它
`scripts/wechat-*-relay.py` 不同：按 `.gitignore` 规则它们从未入库，是操作员本机独立维护的文件，**不属于
本次 macOS 归档搬迁**（WP3 未移动它们）。如果这套 LLDB 文字监控仍在某台 macOS 伴生机上使用，其脚本仍在
原来的相对路径，但下面描述的整体运行方式（`.env` 开关、`WECHAT_GROUP_RELAY_ENDPOINT_TOKEN`）需要对照当前
`feishu-adapter` 的 `/wechat-group-relay` 路由实际鉴权方式重新核实是否仍然一致。
...
```bash
bash scripts/start-wechat-text-relay.sh
tail -f logs/wechat-text-relay.log
```

确认日志和目标群均正常后，才把 `WECHAT_GROUP_RELAY_ENABLED=true` 并重建 adapter。停止：

```bash
bash scripts/stop-wechat-text-relay.sh
```
...
私密配置在本目录的 `.env` 中，权限应保持为仅当前用户可读。它至少包含：
...
已配置两层 LaunchAgent（两份 `.plist` 均已归档在
[`legacy/macos/launchd/`](../../legacy/macos/launchd/)，两个 `.sh` 脚本已归档在
[`legacy/macos/scripts/`](../../legacy/macos/scripts/)）：

1. Homebrew 的 `homebrew.mxcl.colima` 在用户登录后启动 Colima/Docker。
2. `~/Library/LaunchAgents/com.papa.n8n-compose.plist` 在登录后运行
   [`legacy/macos/scripts/start-compose.sh`](../../legacy/macos/scripts/start-compose.sh)。脚本最多等待 Docker 120 秒，然后执行 `docker compose up -d`。
...
### 关键文件（原文档，路径按旧 checkout 布局）

- `compose.yaml`：容器、网络端口、持久卷与环境变量映射（本仓库根目录仍有一份 `compose.yaml`，CI 仍用它起 PostgreSQL 跑 unittest，但不再是生产部署方式）。
- `feishu-adapter/index.mjs`：飞书官方 Node SDK 长连接、转发到 n8n、监控页与 SSE 实现（仍在本仓库，现在由 Windows 运行时监督直接以 `node.exe` 启动）。
- [`legacy/macos/scripts/start-compose.sh`](../../legacy/macos/scripts/start-compose.sh)：等待 Colima 后启动 Compose。
- `/Users/papa/Library/LaunchAgents/com.papa.n8n-compose.plist`：登录自启动配置（未入库，仅存在于原 macOS 机器上）。
- `.env`：仅本机私密变量，禁止提交或分享。
