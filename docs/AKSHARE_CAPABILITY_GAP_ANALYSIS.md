...
日期：2026-08-09。结论基于当前容器内 `akshare==1.18.83` 的实际函数清单和 AKShare 官方文档。AKShare 是公开源聚合库，官方文档覆盖股票、期货、债券、期权、基金、指数、宏观、外汇、能源、另类等大类，但其特别声明把数据定位为研究参考，并提示上游接口可能变化；因此本平台只把它作为 `provider_key=akshare` 的补充/交叉验证层，不替代主源、super 或持牌数据。
