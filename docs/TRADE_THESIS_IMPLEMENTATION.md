...
- 单批最多500只，完整范围按`created_at + thesis_id`稳定排序分页读取，行情证据也按同样上限分批查询；超过500只不再让整轮失败。单批失败时保留已完成批次并明确标记`partial`，面向用户的报告只给中文影响说明，内部错误码留在运行回执与日志。影子评价不重写策略分数或成交记录。持仓意图未绑定时明确`unbound`，不能反推真实买入动机。
