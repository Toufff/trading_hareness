...
- 验收：quant-service 全量 308 项 Python 测试通过，前端 typecheck/Vite build 通过；开盘预检通过，健康接口为 `ok`，迁移为 `20260815_0031`，Prompt Lab 当前无候选且 live_effect=none；`/data-readiness/replay` 配置异步数据库时走原生异步只读仓储（符合历史不回填和数据门禁）。
