...
2. **分析师同步已拆流并完成配置修复，消息流尚未有成功游标。** 报告流按版本/哈希差量，消息流使用持久游标；健康页会把报告流标为 ready、消息流标为 `never_succeeded`，直到真实调度成功推进。此前验证发现游标字段名与远端列表 URL 不一致，导致请求落到 `/analysts/undefined/messages`；已修复生成器与收敛脚本的防回归校验，并于 2026-08-13 盘后重新发布。n8n 主进程与外置 runner 已注入所需运行环境，但连续 10 轮 0 个 429 的验收仍待下一调度窗口。
