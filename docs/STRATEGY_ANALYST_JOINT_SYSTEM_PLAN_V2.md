...
- 验收：quant-service 全量 290 项 Python 测试通过，前端 typecheck/Vite build 通过；开盘预检通过，健康接口为 `ok`，迁移为 `20260815_0031`，Prompt Lab 当前无候选且 live_effect=none（符合历史不回填和数据门禁）。
...
- 2026-08-14 组合风险补强：纸面组合快照按当前精确板块成员拆分暴露，新 entry 会经过 20% 板块集中度门禁；前端展示净值、总/净暴露、回撤和板块暴露。组合级回撤熔断和策略健康自动降级仍受 P3 样本门禁约束。
