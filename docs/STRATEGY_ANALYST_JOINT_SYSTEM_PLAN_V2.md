...
1. **同一研究问题还存在多套口径。** 生产推荐仍使用未复权 `close`，而 `factor_lab` 使用显式复权研究价；实时风险状态尚未真正进入信号准入门禁。
2. **分析师新消息链当前不可用。** n8n 每 15 分钟重复拉取全部历史报告详情，2026-08-13 已连续 10 次因远端 HTTP 429 失败，本地新版消息仍为 0。
...
- n8n 主进程和外置 runner 都显式注入分析师同步必需的非持久化运行环境；旧合并流仍停用，报告流与消息流保持独立。下一交易时段仍须完成连续 10 轮的实际同步验收，才可关闭 429 风险项。
